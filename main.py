...
def split_terms(term):  # this breaks (x)(x+2) into terms x and X+2, or if its a power term the last number is the power
    if "^" in term:
        term = term.replace(")^", "(")
        print("term",term)
    arr = term.split('(')
...
def solver(top, bottom, current_inter_value):  # the solver takes all denom and numerator ,
    top_s = []
    bottom_s = []
    top_s = top[:]  # creates a barrier for the lists so the original does not get modified .
